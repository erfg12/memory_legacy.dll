...
namespace Memory
{
    ...
    /// </summary>
    public class Mem : Imps
    {
        public Proc mProc = new Proc();

		public IntPtr IntPtrAdd( IntPtr ptr, int offSet ) 
		{
			IntPtr ret = new IntPtr( ptr.ToInt64() + offSet );
			return ret;
		}

		public UIntPtr UIntPtrAdd( UIntPtr ptr, uint offSet ) 
		{
			UIntPtr ret = new UIntPtr( ptr.ToUInt64() + offSet );
			return ret;
		}
        ...
        public bool OpenProcess(int pid, out string FailReason)
        {
            ...
            try
            {
                ...
                mProc.Handle = OpenProcess(0x1F0FFF, true, pid);
                ...
                mProc.Is64Bit = false; //Environment.Is64BitOperatingSystem && (IsWow64Process(mProc.Handle, out retVal) && !retVal);
        ...
        /*public ConcurrentDictionary<string, IntPtr> GetModules()
        {
            ...
            foreach (ProcessModule Module in mProc.Process.Modules)
            {
                ...
                if (!string.IsNullOrEmpty(Module.ModuleName) && !mProc.Modules.LastIndexOfKey(Module.ModuleName))
        ...
        public int GetProcIdFromName(string name) //new 1.0.2 function
        {
            ...
            if (name.ToLower().LastIndexOf(".exe") >= 0)
                name = name.Replace(".exe", "");
            if (name.ToLower().LastIndexOf(".bin") >= 0) // test
                name = name.Replace(".bin", "");

            foreach (System.Diagnostics.Process theprocess in processlist)
            {
                if (theprocess.ProcessName.ToLower() == name.ToLower()) //find (name).exe in the process list (use task manager to find the name)
        ...
        public int LoadIntCode(string name, string path)
        {
            try
            {
		...
		// NOT SUPPORTED IN WINDOWS 95/98
        /// <summary>
        ...
        public UIntPtr GetCode(string name, string path)
        {
            ...
            // remove spaces
            if (theCode.LastIndexOf(" ") >= 0)
                theCode = theCode.Replace(" ", String.Empty);

            if (theCode.LastIndexOf("+") < 0 && theCode.LastIndexOf(",")<0)
            {
                try
                {
            ...
            if (newOffsets.LastIndexOf(',') >= 0)
            {
                ArrayList offsetsList = new ArrayList();

                string[] newerOffsets = newOffsets.Split(',');
                foreach (string oldOffsets in newerOffsets)
                {
                    string test = oldOffsets;
                    if (oldOffsets.LastIndexOf("0x")>=0) test = oldOffsets.Replace("0x","");
                    int preParse = 0;
                    if (oldOffsets.LastIndexOf("-")<0)
                ...
                int[] offsets = offsetsList.ToArray(typeof(int)) as int[];

                if (theCode.LastIndexOf("base")>=0 || theCode.LastIndexOf("main")>=0)
                    ReadProcessMemory(mProc.Handle, (UIntPtr)((int)mProc.MainModule.BaseAddress + offsets[0]), memoryAddress, (UIntPtr)8, IntPtr.Zero);
                else if (theCode.LastIndexOf("base")<0 && theCode.LastIndexOf("main")<0 && theCode.LastIndexOf("+")>=0)
                {
                    string[] moduleName = theCode.Split('+');
                    IntPtr altModule = IntPtr.Zero;
                    if (moduleName[0].ToLower().LastIndexOf(".dll")<0 && moduleName[0].ToLower().LastIndexOf(".exe")<0 && moduleName[0].ToLower().LastIndexOf(".bin")<0)
                    {
                        string theAddr = moduleName[0];
                        if (theAddr.LastIndexOf("0x")>=0) theAddr = theAddr.Replace("0x", "");
                    ...
                    ReadProcessMemory(mProc.Handle, (UIntPtr)((int)altModule + offsets[0]), memoryAddress, (UIntPtr)8, IntPtr.Zero);
                }
                else
                    ReadProcessMemory(mProc.Handle, (UIntPtr)(offsets[0]), memoryAddress, (UIntPtr)8, IntPtr.Zero);
                ...
                for (int i = 1; i < offsets.Length; i++)
                {
                    ...
                    ReadProcessMemory(mProc.Handle, base1, memoryAddress, (UIntPtr)8, IntPtr.Zero);
            ...
            else // no offsets
            {
                ...
                if (theCode.ToLower().LastIndexOf("base")>=0 || theCode.ToLower().LastIndexOf("main")>=0)
                    altModule = mProc.MainModule.BaseAddress;
                else if (theCode.ToLower().LastIndexOf("base")<0 && theCode.ToLower().LastIndexOf("main")<0 && theCode.LastIndexOf("+")>=0)
                {
                    string[] moduleName = theCode.Split('+');
                    if (moduleName[0].ToLower().LastIndexOf(".dll")<0 && moduleName[0].ToLower().LastIndexOf(".exe")<0 && moduleName[0].ToLower().LastIndexOf(".bin")<0)
                    {
        ...
        /// <returns></returns>
        /*public UIntPtr Get64BitCode(string name, string path, int size)
        {
            string theCode = "";
            if (path != null)
                theCode = LoadCode(name, path);
            ...
            if (!theCode.LastIndexOf("+") && !theCode.LastIndexOf(","))
            {
                try {
            ...
            if (newOffsets.LastIndexOf(','))
            {
                ...
                {
                    string test = oldOffsets;
                    if (oldOffsets.LastIndexOf("0x")) test = oldOffsets.Replace("0x", "");
                    Int64 preParse = 0;
                    if (!oldOffsets.LastIndexOf("-"))
                ...
                if (theCode.LastIndexOf("base") || theCode.LastIndexOf("main"))
                ...
                else if (!theCode.LastIndexOf("base") && !theCode.LastIndexOf("main") && theCode.LastIndexOf("+"))
                {
                    string[] moduleName = theCode.Split('+');
                    IntPtr altModule = IntPtr.Zero;
                    if (!moduleName[0].ToLower().LastIndexOf(".dll") && !moduleName[0].ToLower().LastIndexOf(".exe") && !moduleName[0].ToLower().LastIndexOf(".bin"))
            ...
            else
            {
                ...
                else if (!theCode.LastIndexOf("base") && !theCode.LastIndexOf("main") && theCode.LastIndexOf("+"))
                {
                    ...
                    {
                        string theAddr = moduleName[0];
                        if (theAddr.LastIndexOf("0x")) theAddr = theAddr.Replace("0x", "");
        ...
        public bool InjectDll(String strDllName)
        {
            ...
            foreach (ProcessModule pm in mProc.Process.Modules)
            {
                if (pm.ModuleName.LastIndexOf("inject",0) >= 0)
                    return false;
            }
        ...
        /// <param name="size">size of the allocated region (DEFAULT is 0x1000)</param>
        ...
        /*public UIntPtr CreateCodeCave(string code, byte[] newBytes, int replaceCount, int size, string file)
        {
            if (replaceCount < 5)
        ...
        /// </summary>
        /*public bool DumpMemory(string file)
        {
        ...
        {
            IntPtr hThread = OpenThread(ThreadAccess.QUERY_INFORMATION, false, (uint)threadId);
            if (hThread == IntPtr.Zero)
